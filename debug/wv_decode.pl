...
#print "********************************\n";
#my $tagListLength = length($inFileContent);
#printf "Length of tag list is $tagListLength\n";
#print $outFileH "$inFileContent";
#close(OUTFILEH);
#exit(1);
#print "********************************\n";
...
while (defined $inFileContent) {
    ...
    if ($tagName =~ /(.+)-(.+)/) {
        ...
        printf("Tag name has length embeded, the length is $tagDataLength \n");
        ...
        printf ("The new tag list length is: $newTagListLength\n");
